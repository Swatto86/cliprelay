...
- **NWG DPI rule**: NWG's builder `.position()`/`.size()` and `set_position()`/`set_size()` already convert logical pixels to physical pixels internally. Always pass unscaled logical pixel values to these APIs. Never pre-multiply by `nwg::scale_factor()` — doing so causes double-scaling on high-DPI displays (e.g. 4K at 150%). The only place `nwg::scale_factor()` should be used is when converting from physical pixels (e.g. `nwg::Monitor::width()`, `window.size()`, `window.position()`) to logical coordinates. Helpers: `logical_window_size()`, `logical_window_position()`, `physical_to_logical_rect()`, `logical_primary_size()`.
